...
EVP_PKEY* Crypto::localKeypair;
  char aesKey3[] = "123456789023456123456789023456";
  char aesIv3[] = "123456789023456";
...
int Crypto::generateAesKey(unsigned char **aesKey, unsigned char **aesIv) {
  ...
  *aesIv = (unsigned char*)malloc(AES_KEYLEN/8);
  ...
  // data for the key and IV. Depending on your use case, you will want to pick one or another.
	...
	memcpy(aesPass,"12345678901234561234567890123456",32);
	unsigned char *aesKey2 = *aesKey;
	unsigned char *aesIv2 = *aesIv;
    if(EVP_BytesToKey(EVP_aes_256_cbc(), EVP_sha256(), aesSalt, aesPass, AES_KEYLEN/8, AES_ROUNDS, aesKey2, aesIv2) == 0) {
      return FAILURE;
    }
    ...
    free(aesSalt);
  #else
  //aesKey = (unsigned char**)aesKey3;
  //aesIv = (unsigned char**)aesIv3;
  memset(*aesKey,'1', AES_KEYLEN/8);
  memset(*aesIv,'1', AES_KEYLEN/8);
  /*
...
int Crypto::aesEncrypt(const unsigned char *message, size_t messageLength, unsigned char **encryptedMessage) {
  ...
  *encryptedMessage = (unsigned char*)malloc(messageLength + AES_BLOCK_SIZE);
  if(encryptedMessage == NULL) {
	...
	unsigned char *aesKey2 = (unsigned char*)malloc(AES_KEYLEN/8);
	unsigned char *aesIV2 = (unsigned char*)malloc(AES_KEYLEN/8);
	strncpy((char*)aesKey2, (const char*)"B374A26A714904AAB374A26A714904AA", AES_KEYLEN/8);
	strncpy((char*)aesIV2, (const char*)"B374A26A714904AA", AES_KEYLEN/16);

  if(!EVP_EncryptInit_ex(aesEncryptContext, EVP_aes_256_cbc(), NULL, (unsigned char*)aesKey2, (unsigned char*)aesIV2)) {
    return FAILURE;
  }
...
int Crypto::aesDecrypt(unsigned char *encryptedMessage, size_t encryptedMessageLength, unsigned char **decryptedMessage) {
  ...
  *decryptedMessage = (unsigned char*)malloc(encryptedMessageLength);
  ...
  // Decrypt it!
  if(!EVP_DecryptInit_ex(aesDecryptContext, EVP_aes_256_cbc(), NULL, (unsigned char*)aesKey, (unsigned char*)aesIv)) {
    return FAILURE;
  }
